...
-- First, clean up duplicate subscriptions before adding unique constraint
...
BEGIN
    ...
    LOOP
        ...
        IF existing_subscription_id IS NULL THEN
            INSERT INTO public.subscriptions (user_id, plan_type, status, created_at, updated_at)
            VALUES (auth_user.id, 'free', 'active', auth_user.created_at, auth_user.updated_at)
            ON CONFLICT DO NOTHING
            RETURNING id INTO existing_subscription_id;
        END IF;

        -- Update user profile with subscription_id if needed
        IF existing_subscription_id IS NOT NULL THEN
            UPDATE public.user_profiles 
            SET subscription_id = existing_subscription_id
            WHERE id = auth_user.id AND subscription_id IS NULL;
        END IF;
    END LOOP;
...
-- Now add the unique constraint after cleaning up duplicates
DO $$
BEGIN
...
EXCEPTION
    WHEN unique_violation THEN
        -- If there are still duplicates, remove them and try again
        DELETE FROM subscriptions 
        WHERE id NOT IN (
            SELECT DISTINCT ON (user_id) id 
            FROM subscriptions 
